...
class Tester:
    def __init__(self, config, test_loader):
        ...
        self.num_color, self.num_style, self.num_season, self.num_category = label_data_file.readlines()[1].split(',')
...
#                correct_part = 0
                ...
                for step, (images, color, style, season, category) in enumerate(self.test_loader):
             ...
             #       part = part.to(self.device)
                    ...
                    color_prediction, style_prediction, season_prediction, category_prediction = self.net(images)
                ...
                print(f'Epoch: {epoch}, Color: {correct_color / len(self.test_loader) / self.batch_size * 100:.4f}%, Style: {correct_style / len(self.test_loader) / self.batch_size * 100:.4f}%, '
                      f'Season: {correct_season / len(self.test_loader) / self.batch_size * 100:.4f}% '
    ...
    def build_model(self):
        self.net: nn.Module = Model(self.backbone, self.num_color, self.num_style, self.num_season,
