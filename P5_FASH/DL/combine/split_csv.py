...
def main():
    ...
    input_file_name = os.path.join(label_dir, 'labels_v3.0.csv')
    ...
    while True:
        ...
        if not line:
            break
        if line.startswith("image_file_name,color,style,season,category"):
            continue

        image_name, color, style, season, category = line.split(',')
        category = category[:-1]
    ...
    file.write(f'color,style,season,category\n')
    file.write(f'{len(color_set)},{len(style_set)},{len(season_set)},{len(category_set)}')
    file.close()
